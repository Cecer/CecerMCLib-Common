 ...
 */
public final class CMLEventFactoryImpl {
	private static final List<CMLArrayBackedEvent<?>> ARRAY_BACKED_EVENTS = new ArrayList<>();

	private CMLEventFactoryImpl() { }

	public static void invalidate() {
		ARRAY_BACKED_EVENTS.forEach(CMLArrayBackedEvent::update);
	}
	...
	public static <T> CMLEvent<T> createArrayBacked(Class<? super T> type, T emptyInvoker, Function<T[], T> invokerFactory) {
		CMLArrayBackedEvent<T> event = new CMLArrayBackedEvent<>(type, emptyInvoker, invokerFactory);
	...
	private static <T> T buildEmptyInvoker(Class<T> handlerClass, Function<T[], T> invokerSetup) {
		...
		//noinspection unchecked
		return (T) Proxy.newProxyInstance(CMLEventFactoryImpl.class.getClassLoader(), new Class[]{handlerClass},
